...
(protected val dbConfigProvider: DatabaseConfigProvider, equipoDAOImpl: ComputerDAOImpl) extends SessionDAO with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  override def add(sesion: Session): Future[String] = {
    ...
    db.run(sesions += sesion).map(res => "Session agregado correctamente").recover {
  ...
  /**
    ...
    */
  override def get(ip: String, fecha: Timestamp): Future[Option[Session]] = {
  ...
  /**
    ...
    */
  override def listAll: Future[Seq[Session]] = {
